...
import org.apache.catalina.webresources.VestigeJarResourceSet;
...
public class TomcatWebAppBuilder implements WebAppBuilder, ContextListener, ParentClassLoaderFinder {
    ...
    public void deployWebApps(final AppInfo appInfo, final ClassLoader classLoader) throws Exception {
        try {
            for (final WebAppInfo webApp : appInfo.webApps) {
                ...
                standardContext.setResources(new StandardRoot() {

                    @Override
                    public void processWebInfLib() {
                        super.processWebInfLib();
                        for (VestigeJar vestigeJar : webApp.vestigeWarDependencies) {
                            classResources.add(new VestigeJarResourceSet(this, "/WEB-INF/classes", vestigeJar, Collections.<VestigeJar> emptyList(), "/"));
                        }
                    }
                });
    ...
    private OpenEJBContextConfig findOpenEJBContextConfig(final StandardContext standardContext) {
